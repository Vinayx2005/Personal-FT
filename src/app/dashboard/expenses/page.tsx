    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-sm text-white/50 mt-0.5">
          What you spent, grouped by month.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-6 mb-6 md:mb-8">
        <div className="card bg-red-900/20 border-red-800/40 !p-4 md:!p-6">
          <p className="text-red-300 text-[10px] md:text-sm font-bold uppercase tracking-wider mb-1 md:mb-2">Total</p>
          <h3 className="text-base md:text-xl font-bold text-red-300 tabular-nums break-words">{formatCurrency(totalExpenses)}</h3>
        </div>
        <div className="card bg-18-orange/15 border-18-orange/40 !p-4 md:!p-6">
          ...
          {inRange.length !== realTxCount && (
            <p className="text-[10px] md:text-xs text-18-dark-text mt-1">
              +{inRange.length - realTxCount} transfer{inRange.length - realTxCount > 1 ? 's' : ''} excluded
            </p>
          )}
        </div>
      </div>
